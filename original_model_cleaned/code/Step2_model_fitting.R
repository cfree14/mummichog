...
# 4 creeks, 3 dispersal models, 4 model structures
...
for(creek in 1:4) {
  ...
  filename <- paste('original_model_cleaned/data/crk', creek, '.dat', sep='')
  dat <- read.data(filename)
  # dat <- readRDS(file.path(datadir, paste0("crk", creek, ".Rds")))
